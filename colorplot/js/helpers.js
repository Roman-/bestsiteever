...
function prependMathToFuncs(s) {
    // let picRx = /\bmona\b/; // picture
    // s = s.replace(picRx, "mona(x,y)");

    let mathFuncs = ['sin', 'cos', 'abs', 'log', 'pow', 'random', 'sign', 'sqrt', 'tan', 'exp', 'max', 'min', 'round'];
    mathFuncs.forEach(function (f) {
        let regex = new RegExp(f + "\\(", 'g');
        s = s.replace(regex, "Math." + f + "(")
    });
    let mathConst = ['PI', 'E'].forEach(function (c) {
        s = s.replace(c, "Math." + c)
    });

    while (s.indexOf('Math.Math.') != -1) {
