...
// returns [x,y] of vector [x0,x1] rotated by theta counter-clockwise
// theta is angle in RADIANS
function vectorRotated(x0,x1, theta) {
    return [
        Math.cos(theta) * x0 - Math.sin(theta) * x1,
        Math.sin(theta) * x0 + Math.cos(theta) * x1,
    ];
}

function floorCeil(x, floor, ceil) {
    return Math.min(ceil, Math.max(x, floor))
}

// returns [x,y] of normalized vector, which length is len
function normalizeVec(x,y,len=1) {
    if (x == 0 && y == 0)
        return [len, 0];
    let currLen = Math.sqrt(x*x + y*y);
    return [x / currLen * len, y / currLen * len];
}

// download plain text
function downloadPlainText(text, fileName = "text.txt") {
