...
function canvasTouched(e) {
    ...
    function clickedOnCell(istarget) {
        ...
        gridScore += (istarget ? 1 : -1); Global.gh = mh835();
        setStatus(gridScore);
        if (istarget) {
...
function generateGradient() { // stopped here
    Global.canvasSize;
    var gradient = Global.ctx.createLinearGradient(0,0,Global.canvasSize,Global.canvasSize);
    gradient.addColorStop("0",   "red");
    gradient.addColorStop("0.33", "magenta");
    gradient.addColorStop("0.66", "yellow");
    gradient.addColorStop("1.0", "red");

    var gradient2 = Global.ctx.createLinearGradient(5,5,Global.canvasSize*0.91,Global.canvasSize);
    gradient2.addColorStop("0",   "red");
    gradient2.addColorStop("0.33", "magenta");
    gradient2.addColorStop("0.66", "yellow");
    gradient2.addColorStop("1.0", "red");

    return {"basic": gradient, "target": gradient2};
}
...
function onSendScoreClick() {
    ...
    sendPost("addtoleaderboard.php", "name="+encodeURIComponent(enteredName)
            +"&score="+gridScore+"&gamesplayed="+Global.gamesPlayed+"&h="+Global.gh,
        function(response) {
